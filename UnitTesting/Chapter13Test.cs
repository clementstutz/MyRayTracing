...
namespace RT.UnitTesting
{
    ...
    class Chapter13Test
    {
        ...
        public void T01_RayMissesCylinder()
        {
            ...
            Vector direction0 = new Vector(0, 1, 0);
            Vector direction1 = new Vector(0, 1, 0);
            Vector direction2 = new Vector(1, 1, 1);
        ...
        public void T02_RayHitsCylinder()
        {
            ...
            Assert.AreEqual(5, xs[0].t);
            Assert.AreEqual(5, xs[1].t);
            xs = c.Intersect(r2);
            Assert.AreEqual(4, xs[0].t);
            Assert.AreEqual(6, xs[1].t);
            xs = c.Intersect(r3);
        ...
        public void T07_EndCaps()
        {
            ...
            Assert.AreEqual(0, c.Intersect(new Ray(p[0], d[0])).Count);
        ...
        public void T09_IntersectConeWithRay()
        {
            ...
            Ray ray = new Ray(new Point(0, 0, -1), new Vector(0, 1, 1).Normalize());
        ...
        public void T11_ConeNormal()
        {
            ...
            n[0] = new Vector(0, 0, 0).Normalize();
            n[1] = new Vector(1, -Math.Sqrt(2), 1).Normalize();
            n[2] = new Vector(-1, 1, 0).Normalize();
        ...
        public void T12_Cylinder()
        {
            ...
            Light light = new Light(new Point(2, 4, -5), new Color(1, 1, 1));
            ...
            c.SetMatrix(Mat4.TranslateMatrix(-1, 0, 0) *
                        Mat4.RotateXMatrix(Math.PI / 2) *
                        Mat4.ScaleMatrix(1, 3, 1));
            ...
            c2.maximum = 1;
            c2.isClosed = true;
            c2.material.pattern = new Patterns.CheckersPattern(new Patterns.SolidColorPattern(new Color(1, 0, 0)),
                                                               new Patterns.SolidColorPattern(new Color(0, 1, 0)));
            c2.material.pattern.matrix = Mat4.ScaleMatrix(0.25, 0.25, 0.25);
            c2.SetMatrix(Mat4.TranslateMatrix(1, 0, 0) *
                         Mat4.RotateXMatrix(Math.PI / 2) *
                         Mat4.ScaleMatrix(1, 3, 1));

            Sphere s = new Sphere();
            s.SetMatrix(Mat4.TranslateMatrix(1, 0, 0) * Mat4.ScaleMatrix(0.5, 0.5, 0.5));

            Sphere s2 = new Sphere();
            s2.SetMatrix(Mat4.TranslateMatrix(-1, 0, 0));

            Camera camera = new Camera(100, 100, Math.PI / 3);
            camera.ViewTransform(new Point(1, 2, -3),
                                 new Point(0, 0, 0),
                                 new Vector(0, 1, 0));
            
            Canvas canvas = Scene.current.Render(camera);
            Save.SaveCanvas(canvas, "Chapter_13_Cylinder");
            Scene.current.Clear();
            Console.WriteLine("Image created.");
        }

        [Test, Order(13)]
        public void T13_Cone()
        {
            if (Scene.current == null)
            {
                new Scene();
            ...
            Light light = new Light(new Point(10, 6, -10), new Color(0.9, 0.9, 0.9));
            ...
            c.material.pattern = new Patterns.CheckersPattern(new Patterns.SolidColorPattern(new Color(0.957, 0.80, 0.604)),
                                                              new Patterns.SolidColorPattern(new Color(0.925, 0.663, 0.333)));
            c.material.pattern.matrix = Mat4.ScaleMatrix(0.25, 0.25, 0.25);
            c.SetMatrix(Mat4.RotateXMatrix(-Math.PI/6) * Mat4.TranslateMatrix(-1, -3, 0) *
            ...
            c2.material.pattern = new Patterns.CheckersPattern(new Patterns.SolidColorPattern(new Color(1, 0, 0)),
                                                               new Patterns.SolidColorPattern(new Color(0, 1, 0)));
            c2.material.pattern.matrix = Mat4.ScaleMatrix(0.25, 0.25, 0.25);
            c2.SetMatrix(Mat4.RotateXMatrix(-Math.PI / 6) * Mat4.TranslateMatrix(1, -3, 0) *
                         Mat4.ScaleMatrix(0.9, 3, 0.9));

            Camera camera = new Camera(100, 200, Math.PI / 3);
            camera.ViewTransform(new Point(0, 2, -8),
                                 new Point(0, -1, 4),
                                 new Vector(0, 1, 0));

            Canvas canvas = Scene.current.Render(camera);
            Save.SaveCanvas(canvas, "Chapter_13_Cone");
            Scene.current.Clear();
            Console.WriteLine("Image created.");
        }

        [Test, Order(14)]
        public void T14_IceCreamCone()
        {
            if (Scene.current == null)
            {
                new Scene();
            ...
            Canvas canvas = Scene.current.Render(camera);
            Save.SaveCanvas(canvas, "Chapter_13_IceCreamCone");
            Scene.current.Clear();
            Console.WriteLine("Image created.");
        }
    }

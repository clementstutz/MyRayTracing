...
namespace RT
{
    public class Cube : RayObject
    {
        ...
        {
            double maxFace = Math.Max(Math.Max(Math.Abs(localPoint.x),
        ...
        public double[] CheckAxis(double origin, double direction)
        {
            ...
            if (Math.Abs(direction) >= Utility.epsilon)
            {
            ...
            {
                //t[0] = tMinNumerator * Double.PositiveInfinity;
                t[0] = tMinNumerator * Utility.Infinity;
                t[1] = tMaxNumerator * Utility.Infinity;
            }
